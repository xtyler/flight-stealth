...
package flight.layouts
{
	...
	public class BoxLayout extends Layout
	{
		...
		public function set padding(value:*):void
		{
			...
			if (target) {
				Invalidation.invalidate(target.display, LayoutEvent.LAYOUT);
			}
			DataChange.change();
		}
		...
		{
			if (target) Invalidation.invalidate(target.display, LayoutEvent.LAYOUT);
		...
		{
			if (target) {
				Invalidation.invalidate(target.display, LayoutEvent.MEASURE);
				Invalidation.invalidate(target.display, LayoutEvent.LAYOUT);
			}
		}
	}

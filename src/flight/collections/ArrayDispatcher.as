...
package flight.collections
{
	...
	dynamic public class ArrayDispatcher extends Array implements IEventDispatcher
	{
		...
		[Bindable(event="queueChangesChange", style="noEvent")]
		...
		[Bindable(event="uniqueChange", style="noEvent")]
		...
		protected function itemChange(index:int, item:Object):void
		{
			if (dispatcher) {
				if (itemChangeEvent) {
					...
					itemChangeEvent = new ListEvent(ListEvent.ITEM_CHANGE, this, false, false, index, index, [item]);
				}
		...
		{
			var index:int = indexOf(event.source);
			itemChange(index, event.source);
		}
		...
		public function addEventListener(type:String, listener:Function, useCapture:Boolean = false, priority:int = 0, useWeakReference:Boolean = true):void
		{
			...
			if (type == ListEvent.ITEM_CHANGE) {
				...
				dispatcher.addEventListener(ListEvent.LIST_CHANGE, onListChange, false, 100);
			}
		}
